'''
This script is to prepare train/val/test data
'''
...
print('Building train/val/test data ...')
train_data_file = './raw_data/train.json'
val_data_file = './raw_data/val_1.json'
val_data_file2 = './raw_data/val_2.json'

train_data = OrderedDict()
val_data = OrderedDict()
val_data2 = OrderedDict()

data_files = [train_data_file, val_data_file, val_data_file2]
data = [train_data, val_data, val_data2]

for idd, data_file in enumerate(data_files):
    split_data = json.load(open(data_file, 'r'))
    for k in split_data.keys():
        video_id = k[2:]
        data[idd][video_id] = split_data[k]

train_data, val_data, val_data2 = data[0], data[1], data[2]
print('Merging Val_1 and Val_2 ...')
merge_keys = list(set(val_data.keys()) | set(val_data2.keys()))

merge_val_data = {}
for k in merge_keys:
    value = {'duration': -1., 'timestamps': [], 'sentences': []}
    if k in val_data.keys():
        value['duration'] = val_data[k]['duration']
        value['timestamps'] = value['timestamps'] + val_data[k]['timestamps']
        value['sentences'] = value['sentences'] + val_data[k]['sentences']
    if k in val_data2.keys():
        value['duration'] = val_data2[k]['duration']
        value['timestamps'] = value['timestamps'] + val_data2[k]['timestamps']
        value['sentences'] = value['sentences'] + val_data2[k]['sentences']

    merge_val_data[k] = value

test_data = merge_val_data
data = [train_data, merge_val_data, test_data]

print('Auto correcting timestamps ...')
for idd, split_data in enumerate(data):
    ids = split_data.keys()
    for idx in ids:
        timestamps = split_data[idx]['timestamps']
        new_timestamps = []
        ...
        split_data[idx]['timestamps'] = new_timestamps

'''
print('Calculating featstamps ...')
for idd, split_data in enumerate(data):
    ids = split_data.keys()
    for idx in ids:
        timestamps = split_data[idx]['timestamps']
        ...
        for stamp in timestamps:
            ...
            feat_start = int(start/2)
            feat_end = int(end/2)
            featstamps.append([feat_start, feat_end])
            
        split_data[idx]['featstamps'] = featstamps
'''

print('#Train: {}, #Val: {}, #Test: {}'.format(len(train_data.items()),
                                               len(merge_val_data.items()),
                                               len(test_data.items())))
...
for idd, split_data in enumerate(data):
    ids = split_data.keys()
    for idx in ids:
        split_data[idx]['encoded_sentences'] = [encode_caption(sent, encoded_vocab)
                                                for sent in split_data[idx]['sentences']]
...
np.save(os.path.join(save_root, 'activitynet_captions_glove_embeds.npy'), glove_vocab)
