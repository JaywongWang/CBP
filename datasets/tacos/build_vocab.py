...
train_data_file = './raw_data/tacos_train_0_74.txt'
val_data_file = './raw_data/tacos_val_100_126.txt'
test_data_file = './raw_data/tacos_test_75_99.txt'

video_info_file = './raw_data/video_info.json'
...
for idd, data_file in enumerate(data_files):
    ...
    for line in lines:
        name, sentences = line.strip().split(':')
        video_name, tail = name.split('.')
        fps = video_info[video_name]
        start_frame, end_frame = tail.split('_')[1:]
...
np.save(os.path.join(save_root, 'tacos_glove_embeds.npy'), glove_vocab)
